...
async def add_products(supplier_id:int,products_details:product_pydanticIn):
    ...
    response = await product_pydantic.from_turtoise_orm(product_obj)
    return {"status":"ok","data": response}
...
@app.put('/product/{id}')
async def updata_product(id:int, update_info: product_pydanticIn):
    ...
    product.unit_prize = update_info['unit_price']
    await product.save()
    resoponse = await product_pydantic.from_tortoise_orm(product)
    return {"status":"ok", "data": resoponse}
...
conf = ConnectionConfig(
    # MAIL_USERNAME = "arjunisthemanth@gmail.com",
